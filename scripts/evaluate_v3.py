#!/usr/bin/env python3
"""
Evaluation script for canopy detection competition - Version 3 (Bbox + Area Caching + Parallel).

Optimized version with bounding box pre-filtering, polygon area caching, and parallel processing.
...
def compute_map_parallel(gt_polygons: List[Polygon],
    ...
    bounding box pre-filtering and polygon area caching.
...
def evaluate_image_parallel(gt_image: Dict[str, Any], pred_image: Dict[str, Any]) -> float:
    """
    ...
    for class_name in classes:
        ...
        # Calculate mAP for this class with bbox pre-filtering and area caching
        class_map = compute_map_parallel(gt_polygons, pred_polygons_with_scores,
...
def evaluate_image_pair(args: Tuple[Dict[str, Any], Dict[str, Any]]) -> Tuple[float, float]:
    """
    ...
    image_map = evaluate_image_parallel(gt_image, pred_image)
...
def calculate_weighted_score(gt_data: Dict[str, Any], pred_data: Dict[str, Any]) -> float:
    ...
    Uses parallel processing for image evaluation.
    ...
    with mp.Pool(processes=num_cores) as pool:
        results = pool.map(evaluate_image_pair, image_pairs)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate canopy detection predictions using organizers' scoring methodology (Optimized v3)"
    )
    parser.add_argument(
        "--gt_json",
        type=Path,
