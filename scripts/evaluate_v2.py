#!/usr/bin/env python3
"""
Evaluation script for canopy detection competition - Version 2 (Bbox + Area Caching).

Optimized version with bounding box pre-filtering and polygon area caching.
...
# Global area cache for polygon areas
_area_cache = {}


def get_polygon_area(polygon: Polygon) -> float:
    """
    Get polygon area with caching.

    Args:
        polygon: Shapely Polygon object

    Returns:
        Polygon area (cached if previously computed)
    """
    # Use polygon coordinates as cache key (more reliable than object ID)
    try:
        # Create a hashable representation of the polygon coordinates
        coords = tuple(polygon.exterior.coords) if not polygon.is_empty else ()

        if coords not in _area_cache:
            _area_cache[coords] = polygon.area

        return _area_cache[coords]
    except Exception:
...
def get_iou(polygon1: Polygon, polygon2: Polygon) -> float:
    ...
    try:
        # OPTIMIZATION 2: Use cached areas
        intersection_polygon = polygon1.intersection(polygon2)
        intersection_area = get_polygon_area(intersection_polygon)

        # Calculate union area using the inclusion-exclusion principle:
        # union_area = area1 + area2 - intersection_area
        area1 = get_polygon_area(polygon1)
        area2 = get_polygon_area(polygon2)
        union_area = area1 + area2 - intersection_area
...
def compute_map(gt_polygons: List[Polygon],
    ...
    # OPTIMIZATION 2: Areas are cached automatically during IoU calculations
...
def calculate_weighted_score(gt_data: Dict[str, Any], pred_data: Dict[str, Any]) -> float:
    ...
    """
    # Clear area cache at the start of evaluation
    clear_area_cache()
    ...
    final_score = sum(weighted_scores) / sum(total_weights)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate canopy detection predictions using organizers' scoring methodology (Optimized v2)"
    )
    parser.add_argument(
        "--gt_json",
        type=Path,
