#!/usr/bin/env python3
"""
Evaluation script for canopy detection competition - Version 4 (All Optimizations).

Fully optimized version with all performance improvements:
...
OPTIMIZATION 2: Polygon area caching
- Cache expensive polygon.area calculations in a dictionary
- Reuse cached areas for intersection/union calculations in IoU
- Expected additional speedup: ~2x

OPTIMIZATION 3: Parallel image processing
- Process multiple images in parallel using multiprocessing
- Each image evaluation is independent so can be parallelized
- Expected additional speedup: ~8x (on 8-core machines)

OPTIMIZATION 4: Spatial indexing (R-tree)
- Use R-tree spatial index to quickly find candidate polygon pairs
- Only test polygons whose bounding boxes overlap significantly
- Expected additional speedup: ~5-10x for dense scenes
...
def get_iou_with_cache(polygon1: Polygon, polygon2: Polygon, area_cache: dict) -> float:
    """
    Calculate Intersection over Union (IoU) for two polygons with area caching.

    Args:
        polygon1: First polygon
        polygon2: Second polygon
        area_cache: Dictionary to cache polygon areas
    ...
    try:
        # OPTIMIZATION 2: Use cached areas with coordinate-based keys
        intersection_polygon = polygon1.intersection(polygon2)

        # Get coordinates as cache keys (more reliable than object IDs)
        poly1_coords = tuple(polygon1.exterior.coords) if not polygon1.is_empty else ()
        poly2_coords = tuple(polygon2.exterior.coords) if not polygon2.is_empty else ()
        inter_coords = tuple(intersection_polygon.exterior.coords) if not intersection_polygon.is_empty else ()

        # Cache areas using polygon coordinates
        if poly1_coords not in area_cache:
            area_cache[poly1_coords] = polygon1.area
        if poly2_coords not in area_cache:
            area_cache[poly2_coords] = polygon2.area
        if inter_coords not in area_cache:
            area_cache[inter_coords] = intersection_polygon.area

        intersection_area = area_cache[inter_coords]
        area1 = area_cache[poly1_coords]
        area2 = area_cache[poly2_coords]
...
def build_rtree_index(polygons: List[Polygon], segmentations: List[List[float]]) -> index.Index:
    """
    Build R-tree spatial index for faster spatial queries.

    Args:
        polygons: List of Shapely Polygon objects
        segmentations: List of segmentation coordinates for bbox extraction

    Returns:
        R-tree index with polygon bounding boxes
    """
    idx = index.Index()
...
def compute_map_rtree(gt_polygons: List[Polygon],
    ...
    all performance improvements including R-tree spatial indexing.
    ...
    # OPTIMIZATION 4: Build R-tree spatial index for GT polygons
    rtree_idx = None
    if gt_segmentations:
        rtree_idx = build_rtree_index(gt_polygons, gt_segmentations)

    # OPTIMIZATION 2: Create local area cache for this computation
    area_cache = {}
    ...
    for pred_idx, pred_polygon in enumerate(pred_polygons_sorted):
        ...
        # OPTIMIZATION 4: Use R-tree to find candidate GT polygons
        candidate_gt_indices = None
        if rtree_idx and pred_bboxes_sorted:
            pred_bbox = pred_bboxes_sorted[pred_idx]
            # Expand search area slightly to account for floating point precision
            expansion = rtree_expansion * max(pred_bbox[2] - pred_bbox[0], pred_bbox[3] - pred_bbox[1])
            expanded_bbox = (
                pred_bbox[0] - expansion,
                pred_bbox[1] - expansion,
                pred_bbox[2] + expansion,
                pred_bbox[3] + expansion
            )
            candidate_gt_indices = list(rtree_idx.intersection(expanded_bbox))
        else:
            # Fallback: test all GT polygons
            candidate_gt_indices = range(num_gt)

        for gt_idx in candidate_gt_indices:
            if gt_matched_map[gt_idx]:  # Skip already matched GT
                continue

            gt_polygon = gt_polygons[gt_idx]
            ...
            # OPTIMIZATION 2: IoU calculation now uses cached areas
            iou = get_iou_with_cache(gt_polygon, pred_polygon, area_cache)
            if iou > best_iou:
                best_iou = iou
                best_gt_idx = gt_idx
...
def evaluate_image_rtree(gt_image: Dict[str, Any], pred_image: Dict[str, Any]) -> float:
    """
    Evaluate a single image by calculating class-wise mAP and averaging.
    Optimized for parallel processing with all optimizations.
    ...
    for class_name in classes:
        ...
        # Calculate mAP for this class with all optimizations
        class_map = compute_map_rtree(gt_polygons, pred_polygons_with_scores,
...
def evaluate_image_pair_rtree(args: Tuple[Dict[str, Any], Dict[str, Any]]) -> Tuple[float, float]:
    """
    Wrapper function for parallel processing of image pairs with all optimizations.

    Args:
        args: Tuple of (gt_image, pred_image)

    Returns:
        Tuple of (image_map_score, image_weight)
    """
    gt_image, pred_image = args
    ...
    image_map = evaluate_image_rtree(gt_image, pred_image)

    return image_map, image_weight
...
def calculate_weighted_score(gt_data: Dict[str, Any], pred_data: Dict[str, Any]) -> float:
    ...
    Uses parallel processing and all optimizations for maximum performance.
    ...
    for gt_image in gt_data["images"]:
        ...
        if file_name in pred_lookup:
            pred_image = pred_lookup[file_name]
            image_pairs.append((gt_image, pred_image))

    if not image_pairs:
        return 0.0

    # OPTIMIZATION 3: Process images in parallel
    num_cores = min(mp.cpu_count(), len(image_pairs))

    with mp.Pool(processes=num_cores) as pool:
        results = pool.map(evaluate_image_pair_rtree, image_pairs)

    # Collect results
    weighted_scores = []
    total_weights = []

    for image_map, image_weight in results:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate canopy detection predictions using organizers' scoring methodology (Fully Optimized v4)"
    )
    parser.add_argument(
        "--gt_json",
        type=Path,
